  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? 'bg-white/97 backdrop-blur-md border-b border-[#E7E3DD] shadow-sm'
          : 'bg-white border-b border-[#E7E3DD]',
      )}
    >
      <Container size="xl">
        <nav className="flex items-center justify-between h-20 sm:h-24">

          {/* Logo — default variant (dark logo on white bg) */}
                              ...
                              {item.children.map((child) => (
                                ...
                                >
                                  <div>
                                    ...
                                    {child.description && (
                                      <div className="mt-0.5 text-xs text-[#64748B] leading-snug">
                                        {child.description}
                                      </div>
                                    )}
                                  </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="lg:hidden overflow-hidden bg-white border-t border-[#E7E3DD]"
          >
            <Container size="xl" className="py-6">
              <ul className="flex flex-col">
                {primaryNav.map((item) => (
                  <li key={item.href} className="border-b border-[#E7E3DD] last:border-0">
                    {item.children ? (
                      <>
                        <button
                          onClick={() =>
                          ...
                          className="w-full flex items-center justify-between py-4 text-base font-medium text-[#334155]"
                        >
                          {item.label}
                          <ChevronDown
                            size={16}
