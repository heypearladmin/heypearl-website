...
export const metadata = {
  ...
  alternates: { canonical: 'https://www.heypearl.io/resources' },
  openGraph: {
    title: 'AI Authority Library — HeyPearl Resources',
    description: 'The definitive educational resource for AI Authority Infrastructure. Learn how AI systems choose which businesses to recommend — and how to build the signals they trust.',
    ...
    <div style={{ background: bg, minHeight: '100vh' }}>
      ...
      <section className="relative overflow-hidden" style={{ borderBottom: `1px solid ${border}`, paddingTop: '5rem', paddingBottom: '4rem' }}>
        <div aria-hidden className="pointer-events-none absolute right-0 top-0 bottom-0 w-[40%] hidden lg:block">
          <Image src="/images/resources/resource-hero.webp" alt="" fill sizes="40vw" className="object-cover object-left-top opacity-25" />
          <div className="absolute inset-0" style={{ background: 'linear-gradient(to right, #F8F5F0 0%, rgba(248,245,240,0.9) 25%, rgba(248,245,240,0.4) 70%, rgba(248,245,240,0.05) 100%)' }} />
        </div>
        <Container size="lg" className="relative z-10">
          ...
          <div className="flex flex-wrap gap-10 mt-10">
            {[
              ...
              <div key={stat.label}>
                <div style={{ fontSize: '1.5rem', fontWeight: 800, color: navy, letterSpacing: '-0.02em' }}>{stat.value}</div>
      ...
      {/* Featured Articles */}
      <section style={{ background: card, borderBottom: `1px solid ${border}`, paddingTop: '4rem', paddingBottom: '4rem' }}>
        <Container size="lg">
          <Reveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px" style={{ border: `1px solid ${border}`, borderRadius: '1rem', overflow: 'hidden' }}>
            {featuredArticles.map((article, i) => (
              <Link
                ...
                className="group block p-8 transition-colors duration-200 hover:bg-[#F8F5F0]"
              ...
              >
                ...
                <p style={{ fontSize: '0.855rem', color: body, lineHeight: 1.65, marginBottom: '1.25rem' }}>
                  {article.excerpt}
                </p>
            ...
            {TOPIC_CLUSTERS.map((cluster, i) => {
              ...
              return (
                <Reveal key={cluster.id} delay={i * 0.08}>
                  <div style={{ background: card, border: `1px solid ${border}`, borderRadius: '0.875rem', padding: '1.5rem' }}>
                    <div className="flex items-start gap-3 mb-4">
                      <div style={{ width: '3px', height: '3rem', background: cluster.color, borderRadius: '999px', flexShrink: 0, marginTop: '0.125rem' }} />
                      <div>
                        <h3 style={{ fontSize: '1rem', fontWeight: 700, color: navy, marginBottom: '0.25rem' }}>{cluster.label}</h3>
                    ...
                    <div style={{ borderTop: `1px solid ${border}`, paddingTop: '1rem' }}>
                      {clusterArticles.map(article => (
                        <Link
                          key={article.slug}
                          href={`/resources/${article.slug}`}
                          className="group flex items-center justify-between py-2.5 hover:opacity-70 transition-opacity"
      ...
      {/* All Articles */}
      <section style={{ background: card, paddingTop: '4rem', paddingBottom: '6rem' }}>
        <Container size="lg">
          <Reveal>
            <div className="mb-8">
              <h2 style={{ fontFamily: 'var(--font-jakarta), Helvetica Neue, Arial, sans-serif', fontSize: '1.5rem', fontWeight: 700, color: navy, letterSpacing: '-0.015em', marginBottom: '0.25rem' }}>
          ...
          <Reveal delay={0.1}>
            <div style={{ border: `1px solid ${border}`, borderRadius: '1rem', overflow: 'hidden' }}>
              {ARTICLE_META.map((article, i) => (
                <Link
                  key={article.slug}
                  href={`/resources/${article.slug}`}
                  className="group flex items-center gap-6 p-6 transition-colors duration-150 hover:bg-[#F8F5F0]"
                  style={{ background: card, borderBottom: i < ARTICLE_META.length - 1 ? `1px solid ${border}` : undefined }}
                >
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, color: muted, width: '1.75rem', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
      ...
      {/* CTA */}
      <section style={{ borderTop: `1px solid ${border}`, background: navy, paddingTop: '5rem', paddingBottom: '5rem' }}>
        <Container size="md">
          <Reveal>
            <div className="text-center">
              ...
              >
                ...
                <span style={{ color: 'rgba(248,245,240,0.65)', fontStyle: 'italic' }}>Building it is step two.</span>
              </h2>
              <p style={{ fontSize: '0.95rem', color: 'rgba(248,245,240,0.55)', lineHeight: 1.7, maxWidth: '28rem', margin: '0 auto 2.5rem' }}>
